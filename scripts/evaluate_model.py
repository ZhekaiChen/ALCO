#!/usr/bin/env python3
"""Run RL evaluation with either SLIME integration or adapter-level validation."""
...
def _build_arg_parser(defaults: Mapping[str, Any]) -> argparse.ArgumentParser:
    ...
    eval_defaults = slime_defaults.get("eval", {}) if isinstance(slime_defaults.get("eval"), Mapping) else {}
    ...
    parser.add_argument("--eval-max-steps-per-episode", type=int, default=None)
    parser.add_argument(
        "--eval-policy",
    ...
    parser.add_argument("--eval-entrypoint", type=str, default=None)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Choose SLIME evaluation pipeline or adapter-level validation pipeline.",
    )
    parser.add_argument(
        "--slime-algorithm",
        "--phase6-algorithm",
        ...
        default=None,
        help="Evaluation algorithm profile: actor_critic (maps to PPO) or grpo.",
    )
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Enable/disable swanlab(wandb-style) tracking for training/evaluation pipeline.",
    )
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Build and persist evaluation plan without launching the runtime.",
    )
    return parser
...
def _build_adapter_settings(args: argparse.Namespace, defaults: Mapping[str, Any]) -> SLIMEAdapterSettings:
    ...
    eval_raw = raw.get("eval", {}) if isinstance(raw.get("eval"), Mapping) else {}
    eval_raw = dict(eval_raw)

    if args.eval_episodes is not None:
        eval_raw["episodes"] = args.eval_episodes
    if args.eval_max_steps_per_episode is not None:
        eval_raw["max_steps_per_episode"] = args.eval_max_steps_per_episode
    if args.eval_policy is not None:
        eval_raw["policy"] = args.eval_policy
    if args.eval_seed is not None:
        eval_raw["seed"] = args.eval_seed
    if args.slime_rollout_function_path is not None:
        eval_raw["slime_rollout_function_path"] = args.slime_rollout_function_path
    if args.slime_eval_function_path is not None:
        eval_raw["slime_eval_function_path"] = args.slime_eval_function_path
    if args.slime_rollout_batch_size is not None:
        eval_raw["slime_rollout_batch_size"] = args.slime_rollout_batch_size
    if args.slime_n_samples_per_prompt is not None:
        eval_raw["slime_n_samples_per_prompt"] = args.slime_n_samples_per_prompt
    if args.slime_eval_dataset_name is not None:
        eval_raw["slime_eval_dataset_name"] = args.slime_eval_dataset_name

    raw = dict(raw)
    raw["eval"] = eval_raw

    if args.eval_entrypoint is not None:
        raw["eval_entrypoint"] = args.eval_entrypoint
...
def run_evaluation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    eval_summary = run_slime_eval(adapter=adapter, settings=adapter_settings)

    run_name = args.run_name or datetime.utcnow().strftime("rl_eval_%Y%m%d_%H%M%S")
    ...
    summary_path = save_json(payload, run_dir / "eval_summary.json")
...
def run_slime_training_evaluation(args: argparse.Namespace) -> dict[str, Any]:
...
run_phase6_evaluation = run_slime_training_evaluation
...
def main() -> None:
    ...
    if args.pipeline == "adapter_validation":
        ...
        print(f"Saved RL eval summary: {result['summary_path']}")
        return

    result = run_slime_training_evaluation(args)
    print(f"Saved RL evaluation summary: {result['summary_path']}")
